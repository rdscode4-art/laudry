...
import '../screens/vendor_order_detail_screen.dart';

class VendorOrderCard extends StatelessWidget {
  final DeliveryOrder order; final VoidCallback onUpdated;
  const VendorOrderCard({super.key, required this.order, required this.onUpdated});
  @override
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () async { await Get.to(() => VendorOrderDetailScreen(order: order)); onUpdated(); },
      child: Container(margin: const EdgeInsets.only(bottom: 14), padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: kCardBg, borderRadius: BorderRadius.circular(16), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.06), blurRadius: 10, offset: const Offset(0, 4))]), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(order.customerName, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15, color: kPrimaryBlue)), Text('${order.id} · Token: ${order.token}', style: TextStyle(fontSize: 12, color: Colors.grey.shade500))])),
        ...
        Row(children: [
          _chip(Icons.checkroom_outlined, '${order.totalItems} items', kAccentBlue),
          const SizedBox(width: 8),
          _chip(Icons.local_laundry_service_outlined, order.service, kOrange),
        ]),
      ])),
    );
  }

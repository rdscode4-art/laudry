...
class _SessionStore {
  ...
  Future<void> save(CustomerSession s) async {
    ...
    if (s.referralCode != null)
      await p.setString(_referralKey, s.referralCode!);
  }
...
class ApiService {
  ...
  DeliveryAuth? get currentDeliveryAuth => isDeliveryLoggedIn
      ? DeliveryAuth(
          token: _deliverySession.token!,
          deliveryId: _deliverySession.deliveryId!,
          name: _deliverySession.name!,
          email: '',
          phone: '',
          kycStatus: _deliverySession.kycStatus!,
        )
      : null;
  ...
  Map<String, String> _getHeaders(String path) {
    ...
    } else if (path.startsWith('/api/driver') &&
        _deliverySession.token != null) {
  ...
  Future<Map<String, dynamic>> _request(
    ...
    switch (method) {
      ...
      case 'PATCH':
        res = await http
            .patch(uri, headers: reqHeaders, body: jsonEncode(body ?? {}))
            .timeout(const Duration(seconds: 15));
        break;
      case 'DELETE':
        res = await http
    ...
    try {
      ...
      throw ApiException(
        res.statusCode >= 500
            ? 'Server error. Is backend running at $baseUrl?'
  ...
  Future<CustomerSession> _signupCustomer(
    String name,
    String email,
    String password, {
    String? vendorId,
  ...
  static Future<void> requestDriverPayout(double amount) async {
    ...
    if (res.statusCode != 200) {
      ...
      throw ApiException(
        b['message'] as String? ?? 'Payout request failed',
        statusCode: res.statusCode,
      );
    }
  }
  ...
  Future<List<VendorInfo>> fetchVendors() async {
    ...
    return list
        .map((e) => VendorInfo.fromJson(e as Map<String, dynamic>))
        .toList();
  }
  ...
  Future<List<Map<String, dynamic>>> fetchOrders() async {
    final query = _session.email != null
        ? {'email': _session.email!}
        : <String, String>{};
  ...
  Future<OrderTimeline> fetchOrderTimeline(String orderId) async {
    ...
    final res = await _request(
      'GET',
      '/api/tracking/orders/$orderId/timeline',
      query: query,
    );
  ...
  Future<Map<String, dynamic>> addComplaint(
    String subject,
    String category,
    String description,
  ) async {
  ...
  Future<void> submitOrderRating(
    ...
    await _request(
      'POST',
      '/api/customer/orders/$orderId/rating',
  ...
  static Future<VendorAuth> loginVendor(String email, String password) async {
    ...
    if (res.statusCode != 200) {
      final b = jsonDecode(res.body) as Map<String, dynamic>;
      throw ApiException(
        b['message'] as String? ?? 'Login failed',
        statusCode: res.statusCode,
      );
    }
    return VendorAuth.fromJson(
      (jsonDecode(res.body) as Map<String, dynamic>)['data']
          as Map<String, dynamic>,
    );
  }

  static Future<VendorAuth> signupVendor(
    String email,
    String password,
    String shopName,
    String phone,
  ...
  static Future<DeliveryAuth> loginDeliveryBoy(
    String login,
    String password,
  ) async {
  ...
  static Future<DeliveryAuth> signupDeliveryBoy(
    ...
    if (res.statusCode != 200 && res.statusCode != 201) {
      final b = jsonDecode(res.body) as Map<String, dynamic>;
      throw ApiException(
        b['message'] as String? ?? 'Signup failed',
        statusCode: res.statusCode,
      );
    }
    final auth = DeliveryAuth.fromJson(
  ...
  static Future<void> uploadDriverKyc({
    ...
    final res = await http.post(
      ...
      headers: {
        ...
        if (instance._deliverySession.token != null)
          'Authorization': 'Bearer ${instance._deliverySession.token}',
      },
      body: jsonEncode({
  ...
  static Future<CustomerSession> signupCustomer(
    String name,
    String email,
    String password, {
    String? vendorId,
    String? referredBy,
  }) => instance._signupCustomer(
    name,
    email,
    password,
    vendorId: vendorId,
  ...
  static Future<void> cancelOrder(String orderId) async {
    ...
    if (res.statusCode != 200) {
      final b = jsonDecode(res.body) as Map<String, dynamic>;
      throw ApiException(
        b['message'] as String? ?? 'Cancel failed',
        statusCode: res.statusCode,
      );
    }
  }
  ...
  static Future<void> rateOrder(
    ...
    if (res.statusCode != 200) {
      final b = jsonDecode(res.body) as Map<String, dynamic>;
      throw ApiException(
        b['message'] as String? ?? 'Rating failed',
        statusCode: res.statusCode,
      );
    }
  }
  ...
  List<DeliveryOrder> _parseDeliveryOrderList(List<dynamic> list) {
    return list.map((e) {
      ...
      return DeliveryOrder(
        ...
        customerLatitude: map['customerLatitude'] != null
            ? (map['customerLatitude'] as num).toDouble()
            : null,
        customerLongitude: map['customerLongitude'] != null
            ? (map['customerLongitude'] as num).toDouble()
            : null,
        vendorLatitude: map['vendorLatitude'] != null
            ? (map['vendorLatitude'] as num).toDouble()
            : (map['vendor_lat'] != null
                  ? (map['vendor_lat'] as num).toDouble()
                  : null),
        vendorLongitude: map['vendorLongitude'] != null
            ? (map['vendorLongitude'] as num).toDouble()
            : (map['vendor_lon'] != null
                  ? (map['vendor_lon'] as num).toDouble()
                  : null),
  ...
  static Future<Map<String, dynamic>> acceptVendorBroadcastOrder(
    String orderId,
  ) async {
    try {
      final res = await instance._request(
        'POST',
        '/api/vendor/accept-order',
  ...
  Future<void> verifyVendorDropoffOtp(String rideId, String otp) async {
    await _request(
      'POST',
      '/api/driver/verify-vendor-dropoff-otp',
      body: {'rideId': rideId, 'otp': otp},
    );
  }
  ...
  Future<void> verifyVendorDispatchOtp(String rideId, String otp) async {
    await _request(
      'POST',
      '/api/driver/verify-vendor-dispatch-otp',
      body: {'rideId': rideId, 'otp': otp},
    );
  }
